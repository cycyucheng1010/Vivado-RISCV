...
////// start copy1
class DmemModule1(implicit p: Parameters) extends LazyModule {
  lazy val module = new DmemModuleImp1(this)
  // FIXME: Unused Diplomacy node needed for conveying the physical address map to the TLB
  val node = TLClientNode(Seq(TLMasterPortParameters.v1(Seq(TLMasterParameters.v1("SHA3_Dmem1")))))
}

class DmemModuleImp1(outer: DmemModule1)(implicit p: Parameters) extends LazyModuleImp(outer)
  with HasCoreParameters {
...
////// start copy2
class DmemModule2(implicit p: Parameters) extends LazyModule {
  lazy val module = new DmemModuleImp2(this)
  // FIXME: Unused Diplomacy node needed for conveying the physical address map to the TLB
  val node = TLClientNode(Seq(TLMasterPortParameters.v1(Seq(TLMasterParameters.v1("SHA3_Dmem2")))))
}

class DmemModuleImp2(outer: DmemModule2)(implicit p: Parameters) extends LazyModuleImp(outer)
  with HasCoreParameters {

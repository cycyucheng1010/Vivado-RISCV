...
class Sha3Accel1(opcodes: OpcodeSet)(implicit p: Parameters) extends LazyRoCC(
    opcodes = opcodes, nPTWPorts = if (p(Sha3TLB).isDefined) 1 else 0) {
  override lazy val module = new Sha3AccelImp1(this)
  val dmemOpt = p(Sha3TLB).map { _ =>
    val dmem = LazyModule(new DmemModule1)
    tlNode := dmem.node
    dmem
  }
}

class Sha3Accel2(opcodes: OpcodeSet)(implicit p: Parameters) extends LazyRoCC(
    opcodes = opcodes, nPTWPorts = if (p(Sha3TLB).isDefined) 1 else 0) {
  override lazy val module = new Sha3AccelImp2(this)
  val dmemOpt = p(Sha3TLB).map { _ =>
    val dmem = LazyModule(new DmemModule2)
    tlNode := dmem.node
    dmem
  }
}
...
class Sha3AccelImp1(outer: Sha3Accel1)(implicit p: Parameters) extends LazyRoCCModuleImp(outer) {
  // 與之前的 `Sha3AccelImp` 實現相同，但改用 CtrlModule1、DmemModule1 和 DpathModule1
  // Suppress DCE to ensure that the module ports are kept consistent
  // between the regular generated Verilog and Sha3BlackBox version
  chisel3.dontTouch(io)
  ...
  if (p(Sha3BlackBox)) {
    ...
    val ctrl = Module(new CtrlModule1(W,S)(p))
    ...
    val dpath = Module(new DpathModule1(W,S)(p))
...
class Sha3AccelImp2(outer: Sha3Accel2)(implicit p: Parameters) extends LazyRoCCModuleImp(outer) {
  // 與之前的 `Sha3AccelImp` 實現相同，但改用 CtrlModule2、DmemModule2 和 DpathModule2
  // Suppress DCE to ensure that the module ports are kept consistent
  // between the regular generated Verilog and Sha3BlackBox version
  chisel3.dontTouch(io)
  ...
  if (p(Sha3BlackBox)) {
    ...
    val ctrl = Module(new CtrlModule2(W,S)(p))
    ...
    val dpath = Module(new DpathModule2(W,S)(p))
...
class WithSha3Accel extends Config ((site, here, up) => {
  ...
  case BuildRoCC => up(BuildRoCC) ++ Seq(
    ...
    (p: Parameters) => {
      val sha3_1 = LazyModule.apply(new Sha3Accel1(OpcodeSet.custom3)(p)) // 第二個加速器
      sha3_1
    },
    (p: Parameters) => {
      val sha3_2 = LazyModule.apply(new Sha3Accel2(OpcodeSet.custom1)(p)) // 第3個加速器
      sha3_2
    }
  )
})
